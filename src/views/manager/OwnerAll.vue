   ...
   methods: {
     saveForm(formName){
       this.$refs[formName].validate((valid) => {
          if (valid) {
             this.$axios.put('http://www.zsw.test:31717/api/managerauth/owner/'+this.editForm.ID,this.editForm).then((response)=>{
     ...
     editRow(row){
      ...
      removeRow() {
         this.$axios.delete('http://www.zsw.test:31717/api/managerauth/owner/'+this.removeID).then((response)=>{
                        console.log(response.data);
                         this.$message(response.data.result)
                         this.getData()
                }).catch((response)=>{
      ...
      getData(){
                  this.$axios.get('http://www.zsw.test:31717/api/managerauth/ownertotal').then((response)=>{
                        console.log(response.data.data.count);
                    this.total = response.data.data.count
                }).catch((response)=>{
                    console.log(response);
                })
                this.$axios.get('http://www.zsw.test:31717/api/managerauth/ownerpage?pageindex='+this.pageindex+'&pagesize='+this.pagesize).then((response)=>{
      ...
      page(currentpage){
                ...
                this.$axios.get('http://www.zsw.test:31717/api/managerauth/ownerpage?pageindex='+currentpage+'&pagesize='+this.pagesize).then((response)=>{
                    this.tableData = response.data.data
                     console.log(response);
                }).catch((response)=>{
                    console.log(response);
